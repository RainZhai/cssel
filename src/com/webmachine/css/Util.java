...
public class Util {
	/**
	 * An array of all the HTML pass-through attributes. The "class" attribute is
	 * left out, since it's a special case.
	 */
	protected static String[] passthroughAttributes = { "onclick", "onkeydown", "onkeyup", "onmousedown", "onmousemove",
			"onblur", "onchange", "ondblclick", "onfocus", "onkeypress", "onreset", "onselect", "onmouseout", "onmouseover",
			"onmouseup", "onsubmit", "onunload", "disabled", "readonly", "alt", "lang", "dir", "tabindex", "accesskey",
			"title", "style", "rows", "cols", "height", "size", "maxlength", "width", "dir", "longdesc", "rules", "frame",
			"border", "cellspacing", "cellpadding", "bgcolor", "usemap", "rel", "rev", "shape", "coords", "hreflang",
			"enctype", "acceptcharset", "accept", "summary", "target", "ismap" };

	public static void addAttribute(Application app, UIComponent component, String key, Object value) {
	...
	public static void writePassthroughAttributes(Map componentAttributes, ResponseWriter writer) throws IOException {
		for (int i = 0; i < passthroughAttributes.length; i++) {
			Object value = componentAttributes.get(passthroughAttributes[i]);
			if (value != null) {
				writer.writeAttribute(passthroughAttributes[i], value, passthroughAttributes[i]);
			}
		}
	...
	public static boolean canModifyValue(UIComponent component) {
		return (component.isRendered() && !getBooleanValue(component.getAttributes().get("readonly"), false) && !getBooleanValue(
				component.getAttributes().get("disabled"), false));
	}
